...
if ( ! function_exists( 'dpstarter_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function dpstarter_posted_on() {
 ...
 * @return bool
 */
function dpstarter_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'dpstarter_categories' ) ) ) {
		...
		set_transient( 'dpstarter_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so dpstarter_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so dpstarter_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in dpstarter_categorized_blog.
 */
function dpstarter_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'dpstarter_categories' );
}
add_action( 'edit_category', 'dpstarter_category_transient_flusher' );
add_action( 'save_post',     'dpstarter_category_transient_flusher' );

/*Post navigation (previous / next post) for single posts.*/
function dpstarter_post_navigation() {
